...
//            $mediaModel->load(Yii::$app->request->post())
    ...
    public function actionGetProductList($term = null, $id = null)
    {
        \Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
        $out = ['results' => ['id' => '', 'text' => '']];
        if (!is_null($term)) {
            $query = new \yii\db\Query;
            $query->select(['products.id AS id', 'name AS text'])
                ->from('products')
                ->where(['like', 'name', $term])
                ->limit(20);
            $command = $query->createCommand();
            ...
            $out['results'] = ['id' => $product->name, 'text' => $product->name];
        }
        return $out;
    }
 ...
 public function actionGetMediaTypeList($term = null, $id = null)
    {
        \Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
        $out = ['results' => ['id' => '', 'text' => '']];
        if (!is_null($term)) {
            $query = new \yii\db\Query;
            $query->select(['media_type.id AS id', 'name AS text'])
                ->from('media_type')
                ->where(['like', 'name', $term])
                ->limit(20);
            $command = $query->createCommand();
        ...
        return $out;
    } public function actionGetAdFormList($term = null, $id = null)
    {
        \Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
        $out = ['results' => ['id' => '', 'text' => '']];
        if (!is_null($term)) {
            $query = new \yii\db\Query;
            $query->select(['advertising_form.name AS id', 'name AS text'])
                ->from('advertising_form')
                ->where(['like', 'name', $term])
                ->limit(20);
            $command = $query->createCommand();
            $data = $command->queryAll();
